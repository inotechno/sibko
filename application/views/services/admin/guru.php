...
$(document).ready(function(){ 
    ...
    function daftar_guru(query){
	    $.ajax({
	        url   : '<?= base_url("admin/Master/view_data_guru")?>',
	        method:"POST",
	        async : false,
	        dataType:'json',
	        data:{query:query},
	        success : function(data){
	            ...
	            if (data.length > 0) {
				                  ...
				                  '<div class="card card-profile">' +
				                    ...
				                    '<div class="row justify-content-center">' +
				                      '<div class="col-lg-3 order-lg-2">' +
				                        '<div class="card-profile-image"><a href="#"><img src="<?= base_url("assets/img/guru/") ?>'+data[i].foto+'" class="rounded-circle"></a></div>' +
				                      '</div>' +
				                    '</div>' +
				                    '<div class="card-header text-center border-0 pt-8 pt-md-4 pb-0 pb-md-4">' +
				                      '<div class="d-flex justify-content-between">' +
				                        '<button data-nik="'+data[i].nik+'" data-nama_lengkap="'+data[i].nama_lengkap+'" data-tempat_lahir="'+data[i].tempat_lahir+'" data-tanggal_lahir="'+data[i].tanggal_lahir+'" data-jenis_kelamin="'+data[i].jenis_kelamin+'" data-agama="'+data[i].agama+'" data-alamat="'+data[i].alamat+'" data-pendidikan="'+data[i].pendidikan+'" data-foto="'+data[i].foto+'" data-hp="'+data[i].hp+'" data-email="'+data[i].email+'" class="btn btn-sm btn-info mr-4 update_guru">Update</button>' +
				                        '<button data-nik="'+data[i].nik+'" class="btn btn-sm btn-danger float-right delete_guru">Delete</button>' +
				                      '</div>' +
				                    '</div>' +
					...
					$('#show_data_guru').html(html);
	            }else{
					$('#show_data_guru').html('<div class="col-12 text-center"><span class="badge badge-pill badge-lg badge-success">Data Tidak Di Temukan</span></div>');
	            }
	        }
	    });
    }
    ...
    $('#add-data').on('click', function () {
   	...
   	$('#show_data_guru').on('click','.update_guru',function(){
    ...
    $('#show_data_guru').on('click','.delete_guru',function(){
        var nik=$(this).attr('data-nik');
    	...
    	Swal.fire({
		  ...
		  confirmButtonText: 'Ya, Hapus!'
			...
			if (result.value) {
				$.ajax({
			        url   : '<?= base_url("admin/Master/delete_guru")?>',
			        method:"POST",
			        async : false,
			        dataType:'json',
			        data:{nik:nik},
    ...
    $('#btn-add').on('click', function () {
    	...
    	$(this).attr('disabled');
	    $.ajax({
	        url: '<?= base_url("admin/Master/save_guru")?>',
	        type: 'POST',
	        dataType: 'JSON',
	        data: formData,
	        cache: false,
	...
	$('#btn-update').on('click', function () {
    	...
    	$(this).attr('disabled');
	    $.ajax({
	        url: '<?= base_url("admin/Master/update_guru")?>',
	        type: 'POST',
	        dataType: 'JSON',
	        data: formData,
	        cache: false,
